package com.danzen.utilities {
	...
	public class Robin extends Sprite {		
		...
		//private var myServer:String = "robinjs-blackbourna.dotcloud.com";  
		//private var myPort:Number = 15228;
		private var myServer:String = "localhost";  
		...
		// getLatestNonNullValue(propertyName:String):String
		// 		returns the last non-null distributed value for the property you pass to it - could be yours
		// getLatestNonNullValueIndex(propertyName:String):Number
		//		returns the index number of the last person to distribute a value for the property you pass to it
		// getLatestNonNullProperties(propertyName:String):Array
		//		returns an array of the last properties to be distributed (sometimes multiple properties are distributed at once)
		...
		private var latestNonNullValues:Object = new Object();
		private var latestNonNullIndexes:Object = new Object();
		private var latestNonNullProperties:Array = [];
		...
		private function fix(e:TimerEvent) {
			getPolicy();
			makeConnection();
		}
		...
		private function getPolicy() {
			...
			mySocket.addEventListener(IOErrorEvent.IO_ERROR, function(e:Event){dispatchEvent(new Event(IOErrorEvent.IO_ERROR));}); // nothing to do	
			mySocket.close();
		}
		...
		private function incomingData(e:DataEvent) {
			...
			//if (connectCheck == false) {											
				...
				for (var i:uint=0; i<lines.length; i++) {
					...
					prop = temp[0];
					if (temp[1] != "") {
						trace (prop, decode(temp[1]).split("|"));
			...
			// wrong? > note... the initial variables sent when instantiating Robin are not used for latest information
			// now they are - the variables sent with history are latest variables
			...
			if (connectCheck == false) {
				...
				for (var iii:uint = 0; iii<latestLines.length; iii++) {					
					if (latestLines[iii] != "") {					
						latestVV = latestLines[iii].split("^"); // [_u_, 0] then [text, 0]
		...
		public function getLatestNonNullValue(w:String):String {		
			if (latestNonNullValues[w]) {
				return latestNonNullValues[w];
			} else {
				return null;
			}	
		}
		
		public function getLatestNonNullValueIndex(w:String):Number {			
			return Number(latestNonNullIndexes[w]);
		}
		
		public function getLatestNonNullProperties(w:String):Array {			
			return latestNonNullProperties;
		}			
		...
		public function dispose() {
			...
			latestNonNullValues = null;
			latestNonNullIndexes = null;
			latestNonNullProperties = null;
			myHistory = null;
