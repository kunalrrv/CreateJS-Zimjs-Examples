package com.danzen.utilities  {
	...
	public class Robin2 extends Sprite {		
		...
		private var myMessage:TextField;
		
		public function Robin2 (
				theMessage:TextField,
				theApplicationName:String,							  
				theMaxPeople:uint = 0, 
			...
			myMessage.appendText("robin\n");
			trace ("hi from Robin d");			
		...
		private function getPolicy() {			
			myMessage.appendText("robin policy\n");
			mySocket.connect(myServer, myPort);
			mySocket.addEventListener(IOErrorEvent.IO_ERROR, function(e:Event){myMessage.appendText("robin - policy error\n" + e + "\n");}); // nothing to do	
			mySocket.close();
			myMessage.appendText("robin policy - closed\n");			
		}
		
		private function makeConnection() {	
		return;
			myMessage.appendText("robin ----- makeConnection\n");
			mySocket.connect(myServer, myPort);
			myMessage.appendText("robin ----- 2 makeConnection\n");
		...
		private function handleMeta(e:Event) {		
			myMessage.appendText("robin - handleMeta\n");
			switch(e.type){
				case 'ioError': 
					trace ("error");
		...
		private function incomingData(e:DataEvent) {
			...
			myMessage.appendText("robin - incomingData\n");
