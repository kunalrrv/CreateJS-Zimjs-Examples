package com.danzen.interfaces.nodeGlobe {
	...
	// new NodeGlobe(data:XML, highlightColor:Number=0xff0000, selectPosition:String="right");
	...
	// can capture all mouse events or use the NodeGlobeEvent which adds a SPIN_DONE event type
	...
	// if you use the NodeGlobeEvent then it is just eventObject.xml that you use
	...
	public class NodeGlobe extends MovieClip {
		...
		private var maxNodeDiameter:Number = 30;
		...
		public function NodeGlobe(theData:XML=null, theDiameter:Number=200, theNodeDiameter:Number=10, theHighlightColor:Number=0x777777, theReorder:Boolean=true, theSelectPosition:String=SELECT_RIGHT) {
			...
			lines = new MovieClip();				
			lines.graphics.lineStyle(2,lineColor,1);
			holder.addChild(lines);
			lines.filters = [new GlowFilter(0xffffff,.8,6,6,2,1,false)];
			lines.alpha = .5
			
			dots = new MovieClip();
			holder.addChild(dots);
			...
			holder.addChild(select)
						
			setData(theData);		
			
			displacementMap = new DisplacementMapFilter();
			displacementMap.mapBitmap = lens(holder.width+50);		
			//displacementMap.mapPoint = new Point(-holder.width/2,-holder.width/2);			
			displacementMap.componentX = BitmapDataChannel.BLUE;
			displacementMap.componentY = BitmapDataChannel.BLUE;
			displacementMap.scaleX = 20;
			displacementMap.scaleY = 20;				
			displacementMap.mode = DisplacementMapFilterMode.CLAMP;
			//addChild(new Bitmap(lens(holder.width)));
			filters = [displacementMap];	
		}		
		
		private function lens(w:Number):BitmapData {
			var WIDTH:Number = w;
			var mat:Matrix = new Matrix();
			mat.createGradientBox(WIDTH,WIDTH,90*Math.PI/180,-(WIDTH / 2),- (WIDTH / 2));
		...
		private function clickGlobe(e:MouseEvent) {			
			dispatchEvent(new NodeGlobeEvent(NodeGlobeEvent.NODE_CLICK, MovieClip(e.target), MovieClip(e.target).xml));
		...
		public function gotoNode(clip:MovieClip) {
			...
			if (d < 0) {d = 360 + d;}
			if (d-holder.rotation > 180) {d = d-360;}
			
			var dur:Number;
			if (clip.id == baseID) {
				d = holder.rotation+360*2;  // spin twice
				dur = 2;
			} else {
				dur = Math.abs(d-holder.rotation) / spinRate;
			}
			...
			TweenLite.to(holder, dur, {rotation:d, ease:Back.easeInOut, onComplete:alignDone, onCompleteParams:[clip]});			
		}
				
		private function alignDone(o:MovieClip) {		
			select.graphics.clear();
			dispatchEvent(new NodeGlobeEvent(NodeGlobeEvent.SPIN_DONE, o, o.xml));
		...
		public function rolloverGlobe(e:MouseEvent):void {
			dispatchEvent(new NodeGlobeEvent(NodeGlobeEvent.NODE_ROLLOVER, e.target, e.target.xml));
		}
		
		public function rolloutGlobe(e:MouseEvent):void {
			dispatchEvent(new NodeGlobeEvent(NodeGlobeEvent.NODE_ROLLOUT, e.target, e.target.xml));
		}		
		...
		private function preprocessRecursiveXML(xml:XML,level:Number=0):void {
			// currently, in recursiveXML() NodeGlobe recursively goes through the data
